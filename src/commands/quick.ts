...
import { handleError, displayPreview, displayBatchResult, displayError, displayWarning, displayProgress, displayProgressResult } from '../utils/display';
import { validateTaskKey } from '../utils/validation';
import type { WorklogEntry, Config, Alias, AIProvider, ProgressCallback } from '../types';
import { t } from '../i18n';

export async function quickCommand(input: string): Promise<void> {
  try {
    const config = requireConfig();
    warnShortInput(input);
    ...
    // Parse with AI
    const entries = await parseWithAI(aiProvider, input, config, aliases, recentTasks);

    // Fill missing tasks interactively
    const resolvedEntries = await resolveUnassignedTasks(entries, aliases);

    // Validate, preview, confirm, log
    await validateAndLog(resolvedEntries, jiraClient);
  } catch (error) {
    handleError(error);
  }
}
...
function warnShortInput(input: string): void {
...
async function resolveUnassignedTasks(
  ...
  for (let i = 0; i < resolved.length; i++) {
    ...
    if (resolvedTasks[entry.activity]) {
      resolved[i] = { ...entry, task: resolvedTasks[entry.activity] };
      continue;
    }
    ...
    resolved[i] = { ...entry, task: resolvedTask };
...
async function validateAndLog(entries: WorklogEntry[], jiraClient: JiraClient): Promise<void> {
  // Validate tasks
  ...
  const progressCallback: ProgressCallback = {
    onStart: (total) => {
      process.stderr.write(`\n${t('display.logging', { count: String(total) })}\n\n`);
    },
    onItem: (current, total, item) => displayProgress(current, total, item),
    onItemResult: (success) => displayProgressResult(success),
  };

  const result = await jiraClient.logBatch(entries, progressCallback);
